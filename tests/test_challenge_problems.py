...
from data import challenge_problems
...
def test_cp_db_collection_names(mongodb_settings):
    base = datacatalog.linkedstores.challenge_problems.ChallengeStore(mongodb_settings)
...
def test_cp_add(mongodb_settings):
    base = datacatalog.linkedstores.challenge_problems.ChallengeStore(mongodb_settings)
    for key, doc, uuid_val in challenge_problems.CREATES:
        resp = base.add_update_document(doc)
        assert resp['uuid'] == uuid_val

def test_cp_update(mongodb_settings):
    base = datacatalog.linkedstores.challenge_problems.ChallengeStore(mongodb_settings)
    for key, doc, uuid_val in challenge_problems.UPDATES:
