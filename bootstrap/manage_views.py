...
from datacatalog import identifiers, linkedstores, mongo, dicthelpers, views
from datacatalog import settings as settings_module
from datacatalog import views as views_module
...
def autobuild(idb, settings):
    views = views_module.aggregations.get_aggregations()
    for viewname, aggregation in views.items():

        try:
            resp = mongo.manage_views.dropView(idb, viewname)
        except Exception as exc:
            logger.warning(exc)

        try:
            mongo.manage_views.createView(idb, viewname, aggregation)
        ...
        if settings['verbose']:
            try:
                built_pipe = mongo.manage_views.getView(idb, viewname).get('cursor').get('firstBatch')[0].get('options').get('pipeline')
                print(json.dumps(built_pipe, indent=2))
            except errors.OperationFailure as err:
                logger.warning(err)

def autodiscover(idb, settings):
    views = views.aggregations.get_aggregations()
    for viewname, aggregation in views.items():
        if settings['verbose']:
            pprint(mongo.manage_views.getView(idb, viewname))
        else:
            print(viewname)
