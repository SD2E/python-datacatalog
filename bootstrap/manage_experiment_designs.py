...
def autobuild(idb, settings):
    ref_store = datacatalog.linkedstores.experiment_design.ExperimentDesignStore(idb)
    ...
    for ref in os.listdir(DATA):
        ...
        for ref in entities:
            ...
            try:
                logger.debug('Registering EXD record {}'.format(ref_abs))
                resp = ref_store.add_update_document(ref, strategy='merge')
                build_log.write('{}\t{}\t{}\t{}\n'.format(
                    'process', resp['experiment_design_id'], resp['uuid'], resp['_update_token']))
...
def dblist(idb, settings):
    ...
    store = datacatalog.linkedstores.experiment_design.ExperimentDesignStore(idb)
    for pipe in store.query({}):
        logger.info('EXD: id={} name="{}" uuid={} updated={}'.format(
            pipe['experiment_design_id'], pipe['title'], pipe['uuid'],
