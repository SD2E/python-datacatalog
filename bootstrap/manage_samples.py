...
DATA = os.path.join(THIS, 'samples')
...
def autobuild(idb, settings):
    ref_store = datacatalog.linkedstores.sample.SampleStore(idb)
    ...
    for ref in os.listdir(DATA):
        ...
        for ref in entities:
            ...
            try:
                ...
                build_log.write('{}\t{}\t{}\t{}\n'.format(
                    'process', resp['sample_id'], resp['uuid'], resp['_update_token']))
                logger.info('Registered {}'.format(resp['sample_id']))
            except Exception:
                logger.exception('Process not added or updated')

def dblist(idb, settings):
    logger.debug('Listing known entities')
    store = datacatalog.linkedstores.sample.SampleStore(idb)
    for pipe in store.query({}):
        logger.info('SMP: id={} name="{}" uuid={} updated={}'.format(
            pipe['sample_id'], '', pipe['uuid'],
