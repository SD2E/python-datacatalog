...
DATA = os.path.join(THIS, 'measurements')
...
def autobuild(idb, settings):
    ref_store = datacatalog.linkedstores.measurement.MeasurementStore(idb)
...
def dblist(idb, settings):
    ...
    store = datacatalog.linkedstores.measurement.MeasurementStore(idb)
