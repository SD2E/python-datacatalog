"""Manage tag annotation definitions
"""
import argparse
import copy
import json
...
DATA = os.path.join(THIS, 'associations')
...
def autobuild(idb, settings):
    ref_store = datacatalog.linkedstores.association.AssociationStore(idb)
    ...
    for ref in os.listdir(DATA):
        ...
        for ref in entities:
            ...
            try:
                logger.debug('Registering association {}'.format(ref_abs))
                resp = ref_store.add_update_document(ref, strategy='merge')
                # build_log.write('{}\t{}\t{}\t{}\n'.format(
                #     'process', resp['experiment_id'], resp['uuid'], resp['_update_token']))
                logger.info('Registered {}'.format(resp['uuid']))
            except Exception:
                logger.exception('Tag not written')

def dblist(idb, settings):
    logger.debug('Listing known entities')
    store = datacatalog.linkedstores.association.AssociationStore(idb)
    for rec in store.query({}):
        logger.info('ASSOC: uuid={} from={} to={} updated={}'.format(
            rec['uuid'], rec['connects_from'][0], rec['connects_to'][0],
